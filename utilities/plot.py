...
class LiveRRTPlot:
    def __init__(self, space, live: bool = True):
        ...
        sx, sy = space.start
        self.ax.plot(sx, sy, marker='D', color='green', markersize=8, linestyle='None', label='Start')

        # Plot goal point and region with marker only
        gx, gy = space.goal
        self.ax.plot(gx, gy, marker='*', color='orange', markersize=12, linestyle='None', label='Goal')
    ...
    def add_node(self, new_point, parent_point):
        if not self.live:
            return
        # Draw connection
        line, = self.ax.plot([parent_point[0], new_point[0]], [parent_point[1], new_point[1]],
                             color='lightgrey', linewidth=1)
        self.edge_lines.append(line)
        # Draw node
        scatter = self.ax.plot(new_point[0], new_point[1], 'bo', markersize=4, linestyle='None')
    ...
    def plot_tree(self, tree: Any):
        ...
        for node in tree.all_nodes_itr():
            ...
            self.ax.plot([parent_pt[0], child_pt[0]], [parent_pt[1], child_pt[1]],
                         color='lightgrey', linewidth=1)
